...
class AdminController {
  addProduct = async (req, res, next, callback) => {
    try {
      ...
      return callback(response("success", "product added succesfully", newproduct));
    } catch (err) {
  ...
  deleteProduct = async (req, callback) => {
    try {
      ...
      for (let i = 0; i < product.image.length; i++) {
        cloudinary.v2.api.delete_resources(product.image.public_id, (res) => {
      ...
      return callback(response("error", "error in deleting product try again later", err));
    }
  }
  editProduct = async (req, res, next, callback) => {
    ...
    if (Product == NaN) {
      return callback(response("error", "product doesn't exist create product first", {}));
    ...
    getorder = async (req, callback) => {
      try {
        ...
        return callback(response("success", "order retrieved succesfully", order));
      }
      catch (err) {
    ...
    getquery = async (req, callback) => {
      try {
        ...
        return callback(response("success", "query retrieved succesfully", query));
      }
      catch (err) {
    ...
    getfeedback = async (req, callback) => {
      try {
        ...
        return callback(response("success", "feedback retrieved succesfully", feedback));
      }
      catch (err) {
        return callback(response("error", "error in extracting query", err));
      }
    ...
    getdashboard = async (req, callback) => {
      try {
        ...
        let totalsum = 0;
        order.forEach(e => {
          totalsum += e.cart.totalCost;

        });


        return callback(response("success", "dashboard data retrieved succesfully", { customers: numOfUser, numoforders: numOfOrders, totalAmount: totalsum }));
