...
class UserController {
  ...
  generateemailotp = async (req, callback) => {
    ...
    if (user.email.isVerified)   callback(response("error", "already verified ", {}));
  ...
  verifyEmail = async (req, callback) => {
    ...
    if (!mongoose.isValidObjectId(id)) callback(response("invalid request", " invalid userId!", {}));
    const user = await db["user"].findById(id);
    if (!user) {
      console.log("in user");
  ...
  payorder = async (req, callback) => {
    try {
      const { amount, razorpayPaymentId, razorpayOrderId, razorpaySignature } =
        req.body;
        console.log(req.user.address+"hello");
  ...
  getorder = async (req, callback) => {
    try {
      ...
      const order = await db['order'].find({user:req.user._id});

      let orders=[];
      for (let i = 0; i < order.length; i++) {
        let element = {};
        element.transactionId=order[i].razorpay.paymentId;
        element.orderId=order[i].razorpay.orderId;
        element.date=order[i].createdAt;
        element.amount=order[i].amount;
        orders.append(element);
        
      }
      return callback(response("success", "order retrieved succesfully", orders));
    }
    catch (err) {
...
for (let index = 0; index < user.cart.items.length; index++) {
  if( user.cart.items[index].productId==productId){
    if(isdurationincreased){
      user.cart.items[index].duration= user.cart.items[index].duration+1
    }else{
      user.cart.items[index].duration= user.cart.items[index].duration-1;
    }
  }
